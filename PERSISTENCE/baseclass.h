...
public:
	...
	//将已经创建的对象写入到文件中，不写入Runtime信息
	friend CArchive& AFXAPI operator<<(CArchive& ar, const CObject& Ob)
	{
		ar.m_bIsStore = TRUE;		
		CObject& Ob2 = (CObject&) Ob;
		Ob2.Serialize(ar);
		return ar;
	}
	//读取到已经创建的对象中
	friend CArchive& AFXAPI operator>>(CArchive& ar, CObject& Ob)
	{
		ar.m_bIsStore = FALSE;		
		Ob.Serialize(ar);
		return ar;
